...
백야 프로토콜은 공동체에 대한 기여활동을 기반으로 화폐토큰과 참정토큰을 발행, 분배함으로써 기여분배권 및 기여생존권, 기여참정권을 보장합니다.

1. 공동체(DAO-consortium)는 기여활동 및 이에 대응되는 토큰가치를 지정하고, 구성원은 이를 행하여 트랜잭션을 발생시킵니다.
2. 기여에 따라 B-token(화폐토큰)의 발행량을 누적하여 생산기반 경제구조를 구축함으로써 기여분배권 및 기여생존권을 보장합니다.
3. 기여에 따라 P-token(참정토큰)을 조건적으로 분배하여 직접민주적 정치구조를 구축함으로써 기여참정권을 보장합니다.
