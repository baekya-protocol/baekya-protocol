...
1. 공통의 목적을 가진 탈중앙화 자율조직은 기여활동 및 이에 대응되는 B-token(화폐)가치를 지정하고, 구성원은 이를 행하여 트랜잭션을 발생시킵니다.
...
![protocol diagram](https://github.com/user-attachments/assets/56b6ed80-0184-4d48-8564-b876a9f21745)
...
4. [모든 DAO-consortium에서는, '제안-투표-검토'의 구성원간 합의과정을 거쳐 의사가 결정 및 반영됩니다.](https://github.com/baekya-protocol/baekya-protocol/blob/main/docs/protocol_overview.md#p-token-political-token)
...
3. 모든 DAO-consortium에서는, '제안-투표-검토'의 구성원간 합의과정을 거쳐 의사가 결정 및 반영됩니다.
   - 제안: 모든 구성원은 1P이상을 담보로 지불하여 안건을 제안할 수 있으며, DAO 내 전체 구성원의 1%만큼의 P-token(최소 1P)이 모이면 투표과정으로 진입합니다. (2주 내로 투표진입 실패시 제안은 소각되고, 담보는 돌려받습니다.)
   - 투표: 구성원은 0.1P를 대가로 찬성, 반대, 기권중 한가지를 선택할 수 있으며, 2주 내로 정족수가 DAO 내 전체 구성원의 40%를 만족하면 투표는 즉시 종료됩니다. 
     - 기권을 제외한 찬성표가 50%이상인 경우에만 제안이 통과되며, 담보를 돌려받게 됩니다.
     - 그 외의 모든 경우에서는 안건과 담보가 모두 소각됩니다. (정족수 실패, 기권 + 반대 50%이상)
   - 검토: 통과된 안건은 DAO 내 운영자(OP)에게 심사되며, OP에게 통과된 안건은 프로토콜 운영DAO인 Ops-DAO(각 DAO OP의 연합체)에 이송, 심사를 거쳐 최종 반영됩니다.
     - OP는 통과된 안건에 대한 거부권을 가지며, 거부시 적절한 이유와 함께 공표하거나 구성원에게 수정안을 제안해야 합니다. (수정안은 즉시 투표과정에 진입합니다.)
     - OP에게 통과되어 Ops-DAO에 이송된 안건은
   ![DAO diagram](https://github.com/user-attachments/assets/70a56acb-d61e-4346-aef7-2f0f8bebb4b4)    
     - OP의 권한은 거부/제안/반영 이며, 토큰 및 제한없이 가능합니다.
     - 구성원은 언제든 OP에 대한 지지를 철회할 수 있으며, 반대지지율이 50%가 되면 즉시 탄핵되므로 OP는 항상 성실을 가해야 하며, 모든 권한행사에 대해 신중을 기울여야 합니다.
